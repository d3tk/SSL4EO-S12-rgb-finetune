...
import wandb
from datasets.SSL4EO.ssl4eo_dataset import SSL4EO
...
def get_args_parser():
    ...
    parser.add_argument('--wandb_project', type=str, default='SSL4EO-RUNS', help='WandB project name')
    parser.add_argument('--wandb_entity', type=str, default='msc_thesis_proj', help='WandB entity (username or team)')
    parser.add_argument('--wandb_run_name', type=str, default='ssl4eo_vits_rgb_finetune', help='Optional run name for WandB')
    parser.add_argument('--wandb_log', action='store_true', help='Enable logging to WandB')
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, images in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.amp.autocast(enabled=args.use_fp16, dtype=torch.bfloat16, device_type='cuda'):
...
class DataAugmentationDINO_S2(object):
    def __init__(self, in_size, global_crops_scale, local_crops_scale, local_crops_number, season='fixed'):
        flip_and_color_jitter = cvtransforms.Compose([
            cvtransforms.RandomHorizontalFlip(p=0.5),
            cvtransforms.RandomApply([
                RandomBrightness(0.4),
...
class DataAugmentationDINO_RGB(object):
    def __init__(self, in_size, global_crops_scale, local_crops_scale, local_crops_number, season='fixed', strategy='single'):
        """
        Args:
    ...
    def __call__(self, image):
        ...
        """
        assert image.shape[1] == 3, f"Expected an RGB image with shape (T, 3, H, W), but got {image.shape}"
        ...
        # **Option 2: Average Over All Seasons**
            ...
            averaged_image = image.mean(axis=0)  # Mean across seasons -> shape (3, H, W)
            x1 = x2 = x3 = np.transpose(averaged_image, (1, 2, 0))  # (H, W, C)

        else:
            raise ValueError(f"Invalid strategy: {self.strategy}. Choose 'single' or 'average'.")
